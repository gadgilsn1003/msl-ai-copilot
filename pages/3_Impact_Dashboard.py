...
<style>
    ...
    .page-header-icon { width: 52px; height: 52px; background: linear-gradient(135deg, rgba(16, 185, 129, 0.12), rgba(16, 185, 129, 0.04)); border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 26px; }
    ...
    .kpi-card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 14px; padding: 22px 20px; }
    .kpi-card:hover { border-color: #10b981; box-shadow: 0 4px 16px rgba(16, 185, 129, 0.1); }
    .kpi-label { font-size: 12px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; }
    .kpi-value { font-size: 32px; font-weight: 800; color: #0f172a; line-height: 1; margin-bottom: 4px; }
    .kpi-change { font-size: 12px; font-weight: 600; }
    .kpi-change.positive { color: #10b981; }
    .demo-banner { background: linear-gradient(135deg, #fffbeb, #fef3c7); border: 1px solid #fbbf24; border-radius: 12px; padding: 14px 20px; margin-bottom: 24px; display: flex; align-items: center; gap: 10px; font-size: 13px; color: #92400e; font-weight: 500; }
    .empty-state { text-align: center; padding: 60px 20px; color: #94a3b8; }
    .empty-state-icon { font-size: 48px; margin-bottom: 16px; opacity: 0.5; }
    .empty-state h3 { font-size: 18px; font-weight: 600; color: #64748b; margin: 0 0 8px 0; }
    .empty-state p { font-size: 14px; margin: 0; }
    .stButton > button { background: linear-gradient(135deg, #10b981, #059669) !important; color: white !important; border: none !important; padding: 10px 20px !important; font-size: 13px !important; font-weight: 600 !important; border-radius: 8px !important; box-shadow: 0 2px 8px rgba(16, 185, 129, 0.2) !important; }
    .stButton > button:hover { transform: translateY(-1px) !important; box-shadow: 0 4px 16px rgba(16, 185, 129, 0.3) !important; }
    ...
    [data-testid="stSidebar"] .stButton > button:hover { background: rgba(16, 185, 129, 0.06) !important; border-color: #10b981 !important; color: #10b981 !important; transform: none !important; }
...
try:
    from backend.database import get_all_interactions
...
if interactions:
    ...
    if PLOTLY_AVAILABLE:
        ...
        with tab_overview:
            ...
            with col_chart1:
                ...
                fig_types = px.bar(
                    ...
                    color="Count", color_continuous_scale=["#bae6fd", "#0ea5e9", "#0369a1"],
                    title="Interactions by Type"
                )
                fig_types.update_layout(
                    showlegend=False, coloraxis_showscale=False,
                    plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                    font=dict(family="Inter", size=12), title_font=dict(size=15, color="#0f172a"),
            ...
            with col_chart2:
                ...
                fig_ta = px.pie(
                    ta_counts, values="Count", names="Therapeutic Area",
            ...
            fig_timeline = px.area(
                monthly, x="month", y="Count", title="Monthly Interaction Volume",
            ...
            fig_timeline.update_layout(
                ...
                xaxis_title="", yaxis_title="Interactions",
            )
        ...
        with tab_kols:
            ...
            fig_kols = px.bar(
                kol_counts, x="Interactions", y="KOL", orientation="h",
                color="Interactions", color_continuous_scale=["#c4b5fd", "#8b5cf6", "#6d28d9"],
            ...
            fig_kol_types = px.bar(
                kol_type_pivot, x="kol_name", y="count", color="type",
            ...
            fig_kol_types.update_layout(
                ...
                xaxis_title="", yaxis_title="Count", legend_title="Type",
            )
            st.plotly_chart(fig_kol_types, use_container_width=True)

        with tab_needs:
            st.markdown("### 🎯 Unmet Needs Theme Analysis")
            st.caption("Keyword frequency analysis from interaction notes.")

            all_notes = " ".join([i.get("notes", "") for i in interactions]).lower()

            keywords = {
                "BBB Penetration": all_notes.count("bbb") + all_notes.count("penetration"),
            ...
            if not keywords_df.empty:
                fig_treemap = px.treemap(
                    keywords_df, path=["Theme"], values="Mentions",
                    title="Unmet Needs Landscape", color="Mentions",
                    color_continuous_scale=["#d1fae5", "#10b981", "#065f46"]
                )
                fig_treemap.update_layout(
                    font=dict(family="Inter", size=12), title_font=dict(size=15, color="#0f172a"),
                    margin=dict(l=0, r=0, t=40, b=0), height=400, coloraxis_showscale=False,
                )
                st.plotly_chart(fig_treemap, use_container_width=True)

                fig_needs_bar = px.bar(
                    keywords_df.head(10), x="Mentions", y="Theme", orientation="h",
                    title="Top Unmet Needs Themes", color="Mentions",
        ...
        with tab_report:
            ...
            if st.button("🧠 Generate Activity Report", use_container_width=True, key="gen_report"):
                if not os.getenv("GOOGLE_API_KEY"):
                    st.warning("Google Gemini API key required for AI report generation.")
                    ...
                    top_kols_list = "\n".join(
                        [f"- {kol}: {count} interactions"
                         for kol, count in zip(kol_counts['KOL'].head(5), kol_counts['Interactions'].head(5))]
                    )
                    top_themes_list = "\n".join(
                        [f"- {theme}: {mentions} mentions"
                         for theme, mentions in zip(keywords_df['Theme'].head(5), keywords_df['Mentions'].head(5))]
                    )
                    report_date = date.today().strftime('%B %d, %Y')

                    report = (
                        f"# MSL Activity Report\n\n"
                        f"## Period: {time_range}\n\n"
                        f"### Summary\n"
                        f"- **Total Interactions:** {total_interactions}\n"
                        f"- **Unique KOLs Engaged:** {unique_kols}\n"
                        f"- **Interaction Types Used:** {interaction_types_count}\n"
                        f"- **Therapeutic Areas Covered:** {therapeutic_areas_count}\n\n"
                        f"### Top Engaged KOLs\n"
                        f"{top_kols_list}\n\n"
                        f"### Key Themes\n"
                        f"{top_themes_list}\n\n"
